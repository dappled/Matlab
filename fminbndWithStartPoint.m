function [xf,fval,exitflag,output] = fminbndWithStartPoint(funfcn,f0,ax,bx,options,varargin)
...
if (nargin==1 || nargin ==0) && nargout <= 1 && isequal(funfcn,'defaults')
    xf = defaultopt;
    return
end
...
if (nargin==1 || nargin ==0)
    if isa(funfcn,'struct')
        ...
        error(message('MATLAB:fminbnd:InputArg'));
    end
end

% Check for non-double inputs
if ~isa(ax,'double') || ~isa(bx,'double')
  error(message('MATLAB:fminbnd:NonDoubleInput'))
end
...
if ischar(maxfun)
    error(message('MATLAB:fminbnd:CharMaxFunEvals'));
end
if ischar(maxiter)
    error(message('MATLAB:fminbnd:CharMaxIter'));
end
...
w = v; xf = v;
d = 0.0; e = 0.0;
fx= funfcn(xf,varargin{:});
if ~isempty(f0)
    fx2 = funfcn(f0,varargin{:});
    if (fx2 < fx)
        xf = f0;
        fx = fx2;
    end
...
if numel(fx) ~= 1
   error(message('MATLAB:fminbnd:NonScalarObj'))
end
...
switch exitflag
    case 1
        msg = ...
            getString(message('MATLAB:fminbnd:OptimizationTerminatedXSatisfiesCriteria', sprintf('%e',tol)));
    ...
    case 0
        if funccount >= maxfun
            msg = getString(message('MATLAB:fminbnd:ExitingMaxFunctionEvals', sprintf('%f',finalf)));
            if print > 0
                disp(' ')
                disp(msg)
            end
        else
            msg = getString(message('MATLAB:fminbnd:ExitingMaxIterations', sprintf('%f',finalf)));
            if print > 0
                disp(' ')
                disp(msg)
            end
    ...
    otherwise
        error(message('MATLAB:fminbnd:InvalidExitflag'))
end
...
if ~isempty(outputfcn)
    switch state
        ...
        otherwise
            error(message('MATLAB:fminbnd:InvalidState'))
    end
end
% Call plot functions
if ~isempty(plotfcns)
...
OUTPUT.algorithm = 'golden section search, parabolic interpolation';
OUTPUT.message = getString(message('MATLAB:fminbnd:OptimizationTerminatedPrematurelyByUser'));
...
if isnan(f)
    error(message('MATLAB:fminbnd:checkfun:NaNFval', localChar( userfcn ), sprintf( '%g', x )));
elseif ~isreal(f)
    error(message('MATLAB:fminbnd:checkfun:ComplexFval', localChar( userfcn ), sprintf( '%g', x )));
end
...
elseif isa(fcn,'function_handle')
    ...
    try
        ...
        strfcn = getString(message('MATLAB:fminbnd:NameNotPrintable'));
    end
end
